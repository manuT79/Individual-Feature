...
class MyPyGame(object):
    ...
    def runIntroWindow(self):
        ...
        while not exitWindow:
            ...
            #optionimage=pygame.image.load('options.png')
            #optionimage1=pygame.image.load('options1.png')
            ...
            #self.button(525,450,optionimage1,optionimage,"options")
        pygame.display.update()
    def update(self):
        pygame.display.update()
        ...
        while not exitGame:
            for event in pygame.event.get():
                if event.type==pygame.QUIT:
                    ...
                    quit()
            if var1 == 1:
                self.ship=pygame.image.load("ship1.png")
            elif var2 == 1:
                self.ship=pygame.image.load("ship2.png")
            elif var3 == 1:
                self.ship=pygame.image.load("ship3.png")
            else:
...
class Gui():
    def __init__(self, root):
        ...
        #mute button
        mute = Button(muteframe, text="Mute", command=lambda a=self.mpg: self.nomusic(), background = "white", width=20)
        mute.pack(side=LEFT)
        #unmute
        unmute = Button(muteframe, text="Unmute", command=lambda a=self.mpg: self.music(), background = "white", width=20)
        unmute.pack(side=LEFT)

        L6=Label(checkboxframe,text='Select a ship model to use:')
        L6.pack(side=TOP)
        ...
        #adds 3 tick boxes and packs them to top
        C7 = Checkbutton(checkboxframe, text = "Red Ship", variable = self.CheckVar7, onvalue = 1, offvalue = 0, height=3,  width = 20)
        C7.pack(side=TOP)
        C8 = Checkbutton(checkboxframe, text = "Yellow Ship", variable = self.CheckVar8, onvalue = 1, offvalue = 0, height=3,  width = 20)
        C8.pack(side=TOP)
        C9 = Checkbutton(checkboxframe, text = "Blue Ship", variable = self.CheckVar9, onvalue = 1, offvalue = 0, height=3,  width = 20)
        C9.pack(side=TOP)
...
#class options():
    #def __init__(self, roots):
      #  self.roots = roots
      #  roots.title('Space Hunter Options')
      #  roots.wm_title("Space Hunter Options")

      #  self.mpg=MyPyGame()

      #  title_frame=Frame(self.roots)
      #  textbox_frame=Frame(self.roots)
      #  checkboxframe=Frame(self.roots)
      #  muteframe=Frame(self.roots)
      #  subtitle_frame1=Frame(self.roots)
      #  fullframe=Frame(self.roots)
      #  unmuteframe=Frame(self.roots)
      #  backframe=Frame(self.roots)
      ...
      #  C1 = Checkbutton(checkboxframe, text = "Ship1", variable = self.CheckVar1, onvalue = 1, offvalue = 0, height=3,  width = 20)
      #  C1.pack(side=TOP)
      #  C2 = Checkbutton(checkboxframe, text = "Ship2", variable = self.CheckVar2, onvalue = 1, offvalue = 0, height=3,  width = 20)
      #  C2.pack(side=TOP)
      #  C3 = Checkbutton(checkboxframe, text = "Ship3", variable = self.CheckVar3, onvalue = 1, offvalue = 0, height=3,  width = 20)
      #  C3.pack(side=TOP)
...
#labels
    #title
       # L1 = Label(title_frame, text="Game Options")
       # L1.pack(side=TOP)
#buttons
    #mute button
      #  mute = Button(muteframe, text="Mute", command=lambda a=self.mpg: self.nomusic(), background = "white", width=20)
    ...
    #back to menu button
     #   backbutton = Button(backframe, text="Back to Menu", command=lambda a=self.mpg: self.BacktoMenu(), background = "green", width=40)
      #  backbutton.pack(side=LEFT)
    #Apply
    #    backbutton = Button(backframe, text="Apply", command=lambda a=self.mpg: self.getinfo(), background = "green", width=40)
     #   backbutton.pack(side=LEFT)
        
   # def BacktoMenu(self):
    #    self.root.withdraw()
   #     self.mpg.runIntroWindow()
   # def music(self):
   #     pygame.mixer.init(frequency=22050, size=-16, channels=2, buffer=4096)
    #    pygame.mixer.music.load("Background Music.mp3")
     #   pygame.mixer.music.play()
   ...
   # def getinfo(self):
       ...
       # ship2i=1
       # ship3i=1
       # ship1i=self.CheckVar1.get()
       # ship2i=self.CheckVar2.get()
       # ship3i=self.CheckVar3.get()
       # self.checkinginfo(ship1i,ship2i,ship3i)
    #def checkinginfo(self,ship1i,ship2i,ship3i):
       # global shipinput
       # shipinput = (ship1i,ship2i,ship3i)
       # self.roots.withdraw()
       # root=tk.Tk()
       # g=Gui(root)
       # root.mainloop()
